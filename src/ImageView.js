...
const CAROUSEL_VIEW = false, DEFAULT_VIEW = true;
    ...
    return (
        <>
            {images.map((image, index) => {
                return (
                    <div key={index} className="relative img-view-container w-64 h-64"
                        onClick={() => {
                        ...
                        <div className="absolute left-0 top-0 w-full h-full">
                            <span>View</span><span>Image</span>
                        </div>
                    </div>
                );
            })}
            <div className="fixed left-0 top-0 w-full h-full carousel-container"
                style={{ zIndex: view === CAROUSEL_VIEW ? 1 : -10, opacity: view === CAROUSEL_VIEW ? 1 : 0 }}>
                <Carousel images={images} imageRefs={imageRefs.current} onClick={(e) => {
