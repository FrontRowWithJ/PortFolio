...
const ProjectBlurb = (props) => {
  ...
  const animateSVG = useCallback(() => {
    if (props.zIndex === 1) {
      ...
      const ids = ["anim", "svg", "img", "li", "gi", "vp", "txt", "abt"];
      const elems = ids.map((id) => document.getElementById(`${id}-${idx}`));
      const [anim, svg, img, li, gi, vp, txt, abt] = elems;
      setDirection(isFirst ? true : !isForward);
      anim.beginElement();
      svg.classList.toggle("animate");
      const offset = 15;
      const refs = [svg, img, li, gi, vp];
      if (value) {
        transformSVG(svg);
        transformImage(img, offset);
        transformLinkedin(li, offset);
        transformGithub(gi, offset);
        transformViewProject(vp, offset);
      } else {
        ...
        refs.forEach((ref) => (ref.style.transform = ""));
      }
      [txt, li, gi, vp, abt].forEach((ref) => toggleNode(value, ref));
      ...
      >
        <nav className="project-info-nav relative">
          ...
          <section className="view-project-section absolute right-0 max-h-full grid">
            ...
            <SocialLink icon="github" fill="none" stroke="white" />
            <SocialLink icon="linkedin" fill="none" stroke="white" />
          </section>
        </nav>
